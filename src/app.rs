...
impl App {
    ...
    pub fn launch_root(&mut self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let session = PtySession::new(
            id.clone(),
            backend,
            self.config.root_command.clone(),
            pty_cols,
            pty_rows,
        );
    ...
    pub fn run(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Clear screen and draw initial sidebar
        write_all(stdout_fd, clear_screen().as_bytes())?;
        self.render_sidebar(stdout_fd)?;

        loop {
            // Get the active session's PTY fd for polling
            let pty_fd = self
                .active_session
                .as_ref()
                .and_then(|id| self.sessions.get(id).ok())
                .map(|s| s.backend().reader_raw_fd());

            // Set up poll on stdin + active PTY
            let stdin_pfd =
                PollFd::new(unsafe { BorrowedFd::borrow_raw(stdin_fd) }, PollFlags::POLLIN);

            let poll_result = if let Some(pty_raw_fd) = pty_fd {
                let pty_pfd =
                    PollFd::new(unsafe { BorrowedFd::borrow_raw(pty_raw_fd) }, PollFlags::POLLIN);
        ...
        match event {
            InputEvent::PassThrough(data) => {
                if let Some(ref id) = self.active_session {
                    let log_event = LogEvent::new(id.clone(), Direction::Input, data.clone());
            ...
            InputEvent::NextTerminal => {
                self.switch_relative(1, stdout_fd)?;
            }
            InputEvent::PrevTerminal => {
                self.switch_relative(-1, stdout_fd)?;
            }
            InputEvent::ShowHelp => {
                self.show_help(stdout_fd)?;
            }
    ...
    fn show_help(&self, stdout_fd: i32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Check if the active session exited and switch if needed.
    /// Returns true if all sessions have exited.
    fn check_session_exit(&mut self) -> bool {
        if let Some(ref id) = self.active_session {
            if let Ok(session) = self.sessions.get(id) {
                if matches!(session.status(), SessionStatus::Exited(_)) {
                    // Try to switch to another running session
                    let next = self
                        .session_order
                        .iter()
                        .find(|s| {
                            *s != id
                                && self
                                    ...
                                    .get(s)
                                    .map_or(false, |sess| {
                                        matches!(sess.status(), SessionStatus::Running)
                                    })
                        })
                        .cloned();

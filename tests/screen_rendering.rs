...
#[test]
fn test_sidebar_does_not_overlap_pty_area() {
    use tttt_tui::SidebarRenderer;
    use tttt_pty::{SessionMetadata, SessionStatus};

    let total_cols: u16 = 120;
    let sidebar_width: u16 = 30;
    let sidebar_start_col = total_cols - sidebar_width + 1; // col 91

    let renderer = SidebarRenderer::new(sidebar_width);
    ...
    let lines = renderer.render(&sessions, Some("pty-1"), total_cols, 24, &[]);

    // Every sidebar line should position cursor at sidebar_start_col or later
    for line in &lines {
        // Extract the cursor position from the line content
        // Format: \x1b[row;colH
        if let Some(pos) = line.content.find("\x1b[") {
            let after_esc = &line.content[pos + 2..];
            if let Some(h_pos) = after_esc.find('H') {
                let params = &after_esc[..h_pos];
                let parts: Vec<&str> = params.split(';').collect();
                if parts.len() == 2 {
                    if let Ok(col) = parts[1].parse::<u16>() {
                        assert!(
                            col >= sidebar_start_col,
                            "sidebar line at row {} starts at col {}, should be >= {}",
                            line.row,
                            col,
                            sidebar_start_col
                        );
...
#[test]
fn test_sidebar_each_line_clears_to_eol() {
    use tttt_tui::SidebarRenderer;
    ...
    for line in &lines {
        assert!(
            line.content.contains("\x1b[K"),
            "sidebar line at row {} should contain clear-to-EOL (\\x1b[K): {:?}",
            line.row,
            line.content
        );
    }
...
#[test]
fn test_full_pipeline_pty_output_then_sidebar() {
    use tttt_pty::{MockPty, PtySession, SessionStatus, SessionMetadata};
    use tttt_tui::SidebarRenderer;

    let total_cols: u16 = 80;
    let sidebar_width: u16 = 30;
    let pty_cols = total_cols - sidebar_width; // 50
    let rows: u16 = 10;

    // Create a mock PTY session at the correct size
    let mut mock = MockPty::new(pty_cols, rows);
    mock.queue_output(b"$ whoami\r\nuser\r\n$ ");
    let mut session = PtySession::new("pty-1".to_string(), mock, "bash".to_string(), pty_cols, rows);

    let _ = session.screen_diff(); // consume initial
    session.pump().unwrap();

    // Get the diff that would be rendered
    let diff = session.screen_diff();
    let cursor = session.cursor_position();

    // Simulate what the harness does:
    // 1. Position at (1,1) and write diff
    // 2. Render sidebar
    // 3. Restore cursor

    let mut output = Vec::new();
    // Step 1: cursor home + diff
    output.extend_from_slice(b"\x1b[1;1H");
    output.extend_from_slice(&diff);

    // Step 2: sidebar
    let sidebar = SidebarRenderer::new(sidebar_width);
    let sessions = vec![session.metadata()];
    let sidebar_lines = sidebar.render(&sessions, Some("pty-1"), total_cols, rows, &[]);
    for line in &sidebar_lines {
        output.extend_from_slice(line.content.as_bytes());
    }

    // Step 3: restore cursor (convert 0-indexed to 1-indexed)
    output.extend_from_slice(format!("\x1b[{};{}H", cursor.0 + 1, cursor.1 + 1).as_bytes());

    // Now parse the full output to see what the terminal would actually show
    let mut display = vt100::Parser::new(rows, total_cols, 0);
    display.process(&output);

    let display_contents = display.screen().contents();

    // The PTY content should be visible in the left portion
    assert!(
        display_contents.contains("$ whoami"),
        "display should show '$ whoami': {:?}",
        display_contents
    );
    assert!(
        display_contents.contains("user"),
        "display should show 'user': {:?}",
        display_contents
    );

    // The sidebar content should be visible in the right portion
    assert!(
        display_contents.contains("TERMINALS"),
        "display should show sidebar header: {:?}",
        display_contents
    );
}

#[test]
fn test_full_pipeline_cursor_position_after_render() {
    use tttt_pty::{MockPty, PtySession};
    use tttt_tui::SidebarRenderer;

    let total_cols: u16 = 80;
    let sidebar_width: u16 = 30;
    let pty_cols = total_cols - sidebar_width;
    let rows: u16 = 10;

    let mut mock = MockPty::new(pty_cols, rows);
    mock.queue_output(b"prompt$ ");
    let mut session = PtySession::new("s1".to_string(), mock, "bash".to_string(), pty_cols, rows);
    ...
    let sidebar = SidebarRenderer::new(sidebar_width);
    let sidebar_lines = sidebar.render(&[], None, total_cols, rows, &[]);
    for line in &sidebar_lines {
        output.extend_from_slice(line.content.as_bytes());
    }
    ...
    output.extend_from_slice(
        format!("\x1b[{};{}H", pty_row + 1, pty_col + 1).as_bytes(),
    );

    let mut display = vt100::Parser::new(rows, total_cols, 0);
    display.process(&output);

    let (final_row, final_col) = display.screen().cursor_position();

    // Cursor should be at the PTY's cursor position (0-indexed)
    assert_eq!(
        (final_row, final_col),
        (pty_row, pty_col),
        "final cursor should match PTY cursor position. PTY=({},{}), Display=({},{})",
        pty_row,
        pty_col,
        final_row,
        final_col
